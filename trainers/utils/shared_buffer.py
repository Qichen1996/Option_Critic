...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        self.cent_obs = np.zeros((self.episode_length + 1, self.n_rollout_threads,
                                  num_agents, *cent_obs_shape), dtype=np.float32)
    ...
    def insert(self, cent_obs, obs, rnn_states_actor, rnn_states_critic,
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        cent_obs = self.cent_obs[:-1].reshape(-1, *self.cent_obs.shape[3:])
        ...
        for indices in sampler:
            ...
            yield cent_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch,\
    ...
    def naive_recurrent_generator(self, advantages, num_mini_batch):
        ...
        cent_obs = self.cent_obs.reshape(-1, batch_size, *self.cent_obs.shape[3:])
        ...
        for start_ind in range(0, batch_size, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                cent_obs_batch.append(cent_obs[:-1, ind])
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        if len(self.cent_obs.shape) > 4:
            cent_obs = self.cent_obs[:-1].transpose(
                1, 2, 0, 3, 4, 5).reshape(-1, *self.cent_obs.shape[3:])
        ...
        for indices in sampler:
            ...
            cent_obs_batch = np.stack(cent_obs_batch, axis=1)
