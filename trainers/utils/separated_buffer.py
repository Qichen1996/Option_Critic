...
class SeparatedReplayBuffer(object):
    def __init__(self, args, obs_space, cent_obs_space, act_space):
        ...
        if type(cent_obs_shape[-1]) == list:
            cent_obs_shape = cent_obs_shape[:1]

        self.cent_obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, *cent_obs_shape), dtype=np.float32)
    ...
    def insert(self, cent_obs, obs, rnn_states, rnn_states_critic, actions, action_log_probs,
               value_preds, rewards, masks, bad_masks=None, active_masks=None, available_actions=None):
        self.cent_obs[self.step + 1] = cent_obs.copy()
    ...
    def chooseinsert(self, cent_obs, obs, rnn_states, rnn_states_critic, actions, action_log_probs,
                     value_preds, rewards, masks, bad_masks=None, active_masks=None, available_actions=None):
        self.cent_obs[self.step] = cent_obs.copy()
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        cent_obs = self.cent_obs[:-1].reshape(-1, *self.cent_obs.shape[2:])
        ...
        for indices in sampler:
            ...
            yield cent_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch
    ...
    def naive_recurrent_generator(self, advantages, num_mini_batch):
        ...
        for start_ind in range(0, n_rollout_threads, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                cent_obs_batch.append(self.cent_obs[:-1, ind])
            ...
            cent_obs_batch = np.stack(cent_obs_batch, 1)
            ...
            cent_obs_batch = _flatten(T, N, cent_obs_batch)
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        if len(self.cent_obs.shape) > 3:
            cent_obs = self.cent_obs[:-1].transpose(1, 0, 2, 3, 4).reshape(-1, *self.cent_obs.shape[2:])
            obs = self.obs[:-1].transpose(1, 0, 2, 3, 4).reshape(-1, *self.obs.shape[2:])
        else:
            cent_obs = _cast(self.cent_obs[:-1])
